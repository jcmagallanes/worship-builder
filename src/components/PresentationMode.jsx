...
import {
  X, ChevronLeft, ChevronRight, Music2, Hash,
  ArrowRight, Gauge, Clock, Maximize2, Minimize2,
  List, Eye
} from 'lucide-react'
...
function KeyBadge({ original, perform }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap justify-center">
      <span className="text-2xl font-bold font-mono bg-white/10 px-4 py-1.5 rounded-xl">
        {original || '—'}
      </span>
      {changed && (
        <>
          <ArrowRight size={20} className="text-amber-300" />
          <span className="text-2xl font-bold font-mono bg-amber-400/20 border border-amber-300/40 text-amber-200 px-4 py-1.5 rounded-xl">
            {perform}
          </span>
          <span className="text-xs font-bold bg-amber-400/30 text-amber-200 border border-amber-300/40 px-2.5 py-1 rounded-full uppercase tracking-widest">
            Key Change
          </span>
        </>
      )}
...
function SlideView({ song, tab, setTab }) {
  const tabs = [
    ...
    ...(song.progressions?.length > 0 ? [{ id: 'progressions', label: 'Numbers', icon: Hash }] : []),
  ]
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="text-center px-6 pt-8 pb-6">
        ...
        {song.artist && (
          <p className="text-lg text-white/50 mt-2 font-medium">{song.artist}</p>
        )}
      </div>
          ...
          <div className="space-y-4 max-w-xl mx-auto">
            ...
            {song.originalKey && (
              <div className="bg-white/8 rounded-2xl p-5 text-center">
                <p className="text-xs font-bold uppercase tracking-widest text-white/40 mb-3">Key</p>
                <KeyBadge original={song.originalKey} perform={song.performKey} />
              </div>
            )}
              ...
              <div className="grid grid-cols-2 gap-3">
                {song.tempo && (
                  <div className="bg-white/8 rounded-2xl p-4 text-center">
                    <p className="text-xs font-bold uppercase tracking-widest text-white/40 mb-1">Tempo</p>
                    <p className="text-3xl font-bold text-white font-mono">{song.tempo}</p>
                    <p className="text-white/40 text-xs mt-1">BPM</p>
                  </div>
                )}
                {song.timeSignature && (
                  <div className="bg-white/8 rounded-2xl p-4 text-center">
                    <p className="text-xs font-bold uppercase tracking-widest text-white/40 mb-1">Time</p>
                    <p className="text-3xl font-bold text-white font-mono">{song.timeSignature}</p>
                  </div>
                )}
              </div>
            )}
            ...
            {song.notes && (
              <div className="bg-amber-400/10 border border-amber-300/20 rounded-2xl p-5">
                <p className="text-xs font-bold uppercase tracking-widest text-amber-300/70 mb-2">Performance Notes</p>
                <p className="text-white/80 text-base leading-relaxed">{song.notes}</p>
              </div>
            )}
        ...
        {tab === 'progressions' && song.progressions?.length > 0 && (
          <div className="max-w-xl mx-auto space-y-3">
            {song.progressions.map((prog, i) => (
              <div key={prog.id || i} className="bg-white/8 rounded-2xl px-5 py-4 flex items-baseline gap-4">
                {prog.label && (
  ...
  return (
    <div className="absolute inset-0 z-10 bg-[#0f0e0c]/95 backdrop-blur-sm flex flex-col">
      <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
        ...
        <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-xl text-white/60">
          <X size={18} />
        </button>
      </div>
      <div className="overflow-y-auto flex-1 p-3">
        {songs.map((song, i) => (
          ...
          >
            <span className={`text-xs font-bold w-6 text-center shrink-0 ${i === currentIdx ? 'text-white' : 'text-white/30'}`}>
              {i + 1}
            </span>
...
export default function PresentationMode({ songs, initialIndex = 0, onClose }) {
  ...
  const goPrev = useCallback(() => { if (!isFirst) goTo(currentIdx - 1) }, [currentIdx, isFirst, goTo])
  const goNext = useCallback(() => { if (!isLast) goTo(currentIdx + 1) }, [currentIdx, isLast, goTo])
  ...
  return (
    ...
    >
      ...
      <div className="h-0.5 w-full" style={{ background: `linear-gradient(90deg, transparent, ${accentColor}66, transparent)` }} />

      {/* Header controls */}
      <div className={`flex items-center justify-between px-4 py-3 transition-opacity duration-300 no-print ${showControls ? 'opacity-100' : 'opacity-0'}`}>
      ...
      {/* Bottom navigation */}
      <div className={`flex items-center justify-between px-4 pb-6 pt-2 transition-opacity duration-300 no-print ${showControls ? 'opacity-100' : 'opacity-0'}`}>
        {/* Prev */}
        <button
          onClick={goPrev}
          disabled={isFirst}
        ...
        <div className="flex gap-1.5 items-center">
          {songs.map((_, i) => (
            <button
              ...
              style={{
                background: i === currentIdx ? accentColor : 'rgba(255,255,255,0.25)'
              }}
            />
          ))}
        </div>
      ...
      <div className={`absolute bottom-20 left-1/2 -translate-x-1/2 text-white/25 text-xs font-medium transition-opacity duration-1000 pointer-events-none no-print ${showControls ? 'opacity-100' : 'opacity-0'}`}>
